...
class HIVTreatment(object):
    ...
    **REFERENCE:**
    ...
    def __init__(self, logspace=True, dt=5, model_derivatives=None, perturb_params=False,perturb_rate = 0.0, \
        p_T1=0, p_T2=0, p_T1s=0, p_T2s=0, p_V=0, p_E=0, noise_stds=[0.3, 0.1, 0.3, 0.1, 0.3, 0.1], T=1000, **kw):
        """
    ...
    def is_done( self, episode_length=None, **kw ):
        """Check if we've finished the episode."""
        if episode_length: 
            return self.t >= episode_length 
        else:
            return self.t >= self.T / self.dt
    ...
    def calc_reward(self, action=0, state=None, **kw ):
        ...
        reward = int(np.log10(V) < 4 and np.log10(E) > 1.5)
